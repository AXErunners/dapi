...
const AxeCoreRpcError = require('../../errors/AxeCoreRpcError');
const constants = require('./constants');
const config = require('../../config');

const client = new RpcClient(config.axecore.rpc);
...
const generate = amount => new Promise((resolve, reject) => { // not exist?
  client.generate(amount, (err, r) => {
    if (err) {
      reject(new AxeCoreRpcError(err.message));
    } else {
      resolve(r.result);
    }
  });
...
const getMnListDiff = (baseBlockHash, blockHash) => new Promise((resolve, reject) => {
  client.protx(constants.AXECORE_RPC_COMMANDS.protx.diff, baseBlockHash, blockHash, (err, r) => {
    if (err) {
      reject(new AxeCoreRpcError(err.message));
    } else {
      resolve(r.result);
