 ...
 * This module is a temporary solution until proper index in axecore is provided.
 */

const union = require('lodash/union');
const EventEmitter = require('events');

let axecoreRpcClient;
let logger;
...
async function processBlock(blockHeight) {
  const blockHash = await axecoreRpcClient.getBlockHash(blockHeight);
  const block = await axecoreRpcClient.getBlock(blockHash);
  let nextBlockExists = false;
  if (block) {
    nextBlockExists = !!block.nextblockhash;
    const transactionHashes = block.tx;
    let users = await Promise.all(transactionHashes.map(transactionHash => axecoreRpcClient
...
function start({ axeCoreRpcClient: rpcClient, axeCoreZmqClient, log }) {
  axecoreRpcClient = rpcClient;
  logger = log;
  safeUpdateUsernameIndex();
  axeCoreZmqClient.on(axeCoreZmqClient.topics.hashblock, safeUpdateUsernameIndex);
}
