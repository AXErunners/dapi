...
const p2p = require('@axerunners/axecore-p2p');
...
class SpvService {
  constructor() {
    this.config = config.axecore.p2p;
    this.clients = [];
  ...
  createNewClient(filter) {
    ...
    return new Promise((resolve, reject) => {
      ...
      peer.once('disconnect', () => {
        ...
        reject(new Error('Not able to establish p2p connection to axecore'));
      });
    });
  }
  ...
  initListeners(client, filter) {
    ...
    peer.on('merkleblock', (message) => {
      ...
      // Discussion: https://axerunners.atlassian.net/browse/EV-847
