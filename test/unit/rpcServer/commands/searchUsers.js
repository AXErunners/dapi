...
describe('searchUsers', () => {
  ...
  it('Should return search results', async () => {
    ...
    const users = await searchUsers({ pattern: 'Axe', limit: 10, offset: 0 });
  ...
  it('Should throw an error if arguments are not valid', async () => {
    ...
    expect(spy.callCount).to.be.equal(0);
    await expect(searchUsers({ pattern: 'Axe', offset: -1, limit: 10 })).to.be.rejectedWith('offset should be >= 0');
    expect(spy.callCount).to.be.equal(0);
    await expect(searchUsers({ pattern: 'Axe', offset: 10, limit: -1 })).to.be.rejectedWith('limit should be >= 1');
    expect(spy.callCount).to.be.equal(0);
    await expect(searchUsers({ pattern: 'Axe', offset: 10, limit: 0.5 })).to.be.rejectedWith('should be integer');
    expect(spy.callCount).to.be.equal(0);
    await expect(searchUsers({ pattern: 'Axe', offset: 0.5, limit: 10 })).to.be.rejectedWith('should be integer');
    expect(spy.callCount).to.be.equal(0);
    await expect(searchUsers({ pattern: 'Axe', offset: '10', limit: 10 })).to.be.rejectedWith('should be integer');
    expect(spy.callCount).to.be.equal(0);
    await expect(searchUsers({ pattern: 'Axe', offset: 20, limit: '10' })).to.be.rejectedWith('should be integer');
    expect(spy.callCount).to.be.equal(0);
